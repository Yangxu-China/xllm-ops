...
if (BUILD_OPEN_PROJECT)
    ...
    if (NOT ENABLE_BUILT_IN)
        install(TARGETS cust_opapi
                LIBRARY DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/op_api/lib
        )
    endif()
    ...
    if (NOT ENABLE_BUILT_IN)
        install(TARGETS cust_proto
                LIBRARY DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/op_proto/lib/linux/${CMAKE_SYSTEM_PROCESSOR}
        )
    endif()
    ...
    if (NOT ENABLE_BUILT_IN)
        install(TARGETS cust_opmaster
                LIBRARY DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/op_impl/ai_core/tbe/op_tiling/lib/linux/${CMAKE_SYSTEM_PROCESSOR}
        )
    endif()
    ...
    if (NOT ENABLE_BUILT_IN)
        install(FILES ${compat_optiling_file}
                DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/op_impl/ai_core/tbe/op_tiling
        )
    endif()
...
foreach (OP_DEPEND_DIR ${OP_DEPEND_DIR_LIST})
    ...
    if ( "${OP_DEPEND_DIR}" MATCHES ".*moe_inplace_index_add_with_sorted.*")
       list(APPEND OP_DIR_LIST ${OPS_XLLM_DIR}/moe/3rd/moe_inplace_index_add_with_sorted)
    endif()
endforeach ()
...
if (BUILD_OPEN_PROJECT)
    ...
    add_dependencies(cust_proto ops_xllm_proto_headers)

    if (NOT ENABLE_BUILT_IN)
        install(FILES ${generate_proto_headers}
                DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/op_proto/inc OPTIONAL
        )
    endif()
    ...
    target_sources(cust_proto PRIVATE
        ${ASCEND_GRAPH_CONF_DST}/ops_proto_xllm.cpp
    )
    ...
    if (generate_proto_srcs)
        ...
        add_dependencies(opsproto_rt2.0 ops_xllm_proto_headers)
    endif ()
...
add_library(ops_xllm_proto_headers INTERFACE)

target_include_directories(ops_xllm_proto_headers INTERFACE
...
if ((NOT BUILD_OPEN_PROJECT) AND ("${PRODUCT_SIDE}" STREQUAL "device"))
    ExternalProject_Add(extern_opbuild_gen_xllm
    ...
    add_dependencies(ops_xllm_proto_headers extern_opbuild_gen_xllm)
else()
    add_dependencies(ops_xllm_proto_headers opbuild_gen_default opbuild_gen_inner opbuild_gen_exc)
endif ()
...
if (NOT BUILD_OPEN_PROJECT)
    if (generate_proto_srcs)
        install_package(
                PACKAGE ops_adv
                TARGETS ops_xllm_proto_headers
...
foreach (op_dir ${OP_DIR_LIST})
    ...
    foreach (op_depend_dir ${${_op_name}_depends})
        set(CURRENT_KERNEL_DIR "${OPS_XLLM_DIR}/${op_depend_dir}/op_kernel")
        file(GLOB KERNEL_SUB_DIRS RELATIVE "${CURRENT_KERNEL_DIR}" "${CURRENT_KERNEL_DIR}/*")
...
if (BUILD_OPEN_PROJECT)
    ...
    add_dependencies(prepare_build generate_xllm_adapt_py generate_compile_cmd)
    ...
    add_dependencies(tbe_ops_json_info generate_xllm_adapt_py)
endif ()

# ------------------------------------------------ opp kernel ------------------------------------------------
if (ENABLE_OPS_KERNEL)
    add_custom_target(ops_xllm_kernel ALL)
    add_custom_target(ops_xllm_config ALL)
    add_dependencies(ops_xllm_kernel ops_xllm_config)
...
if (NOT ENABLE_BUILT_IN AND BUILD_OPEN_PROJECT)
    ...
    add_custom_command(OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/scripts/install.sh ${CMAKE_CURRENT_BINARY_DIR}/scripts/upgrade.sh
            ...
            COMMAND sed -i "s/vendor_name=customize/vendor_name=${VENDOR_NAME}_xllm_math/g" ${CMAKE_CURRENT_BINARY_DIR}/scripts/*
    )
    ...
    install(FILES ${version_info_file}
            DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/
    )
    ...
    if (ENABLE_BUILD_PKG)
      if (EXISTS ${ASCEND_CMAKE_DIR}/makeself.cmake)
        ...
        set(CPACK_MAKESELF_PATH ${OPS_XLLM_DIR}/../third_party/makeself)
