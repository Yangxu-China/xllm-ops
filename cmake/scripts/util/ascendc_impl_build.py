...
PYPTO_COMPILE_OP_API = """
    ...
    _spec = _ilu.spec_from_file_location("_pypto_name_check", src)
    if _spec and _spec.loader:
        _chk_mod = _ilu.module_from_spec(_spec)
        import sys as _sys
        _sys.modules["_pypto_name_check"] = _chk_mod
        _spec.loader.exec_module(_chk_mod)
        from pypto_pro.runtime.jit import _TileJitKernel as _TK
        if isinstance(getattr(_chk_mod, origin_func_name, None), _TK):
            _resolved_func = origin_func_name
        elif isinstance(getattr(_chk_mod, origin_func_name + "_kernel", None), _TK):
            _resolved_func = origin_func_name + "_kernel"
        else:
            raise RuntimeError("no @pl.jit kernel named '{{}}' or '{{}}_kernel' in {{}}".format(origin_func_name, origin_func_name, src))
        _sys.modules.pop("_pypto_name_check", None)
    else:
        _resolved_func = origin_func_name
    pypto_compile_op(src, _resolved_func, op_info, options, code_channel, '{}', {})
"""
