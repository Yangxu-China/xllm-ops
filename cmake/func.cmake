...
function(add_ops_info_target)
    ...
    if (ENABLE_BUILT_IN)
        set(OPS_INFO_JSON ${ASCEND_AUTOGEN_DIR}/aic-${OPINFO_COMPUTE_UNIT}-ops-info-xllm.json)
    else()
    ...
    if (ENABLE_BUILT_IN)
        install(FILES ${OPS_INFO_JSON}
                DESTINATION ops_xllm/built-in/op_impl/ai_core/tbe/config/${OPINFO_COMPUTE_UNIT} OPTIONAL
        )
    else()
        install(FILES ${OPS_INFO_JSON}
                DESTINATION packages/vendors/${VENDOR_NAME}_xllm_math/op_impl/ai_core/tbe/config/${OPINFO_COMPUTE_UNIT} OPTIONAL
        )
    endif()
endfunction()
...
function(add_bin_compile_target)
    ...
    if (ENABLE_BUILT_IN)
        set(_INSTALL_DIR ops_xllm/built-in/op_impl/ai_core/tbe/kernel)
    else()
        set(_INSTALL_DIR packages/vendors/${VENDOR_NAME}_xllm_math/op_impl/ai_core/tbe/kernel)
    endif()
    ...
    foreach(bin_script ${compile_scripts})
        ...
        if (NOT TARGET ${op_file})
            ...
            add_dependencies(ops_xllm_kernel ${op_file})
        endif ()
        ...
        if (NOT TARGET ${OP_TARGET_NAME})
            ...
            if (ENABLE_BUILT_IN)
                install(DIRECTORY ${OP_BIN_OUT_DIR}
                        DESTINATION ${_INSTALL_DIR}/${BINARY_COMPUTE_UNIT}/ops_xllm OPTIONAL
                )
                install(FILES ${BIN_OUT_DIR}/${op_file}.json
                        DESTINATION ${_INSTALL_DIR}/config/${BINARY_COMPUTE_UNIT}/ops_xllm OPTIONAL
                )
            else()
                install(DIRECTORY ${OP_BIN_OUT_DIR}
    ...
    if (_ops_target_list)
        ...
        add_dependencies(ops_xllm_config ${OPS_CONFIG_TARGET})
...
function(add_static_ops)
    ...
    set(prepare_ops_adv_static_target prepare_ops_xllm_static)
