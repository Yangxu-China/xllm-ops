...
### 目标
- 实现并验证 `x_attention_v2` 算子（xllm-ops 仓库），包括 `x_attention_v2_kernel` 入口重命名、内置编译/测试验证，以及一个新的 JIT 直调测试文件 `test_x_attention_v2_jit.py`。

### 限制与偏好
- PyPTO-pro 仅支持 A5（Ascend950PR），不支持 A3
- 构建系统需要 1 个内核入口用于内置二进制交付
- 不要修改 CANN pip 包内容 (`pypto_pro`)
- `tiling_key` 的 `if` 分支可以条件性地声明不同数量/形状的 `make_tile_group`（每个 `tiling_key` 变体独立编译，死分支会被折叠）
- Golden 缓存目录必须按算子名称进行子目录划分：`golden_cache/x_attention/`
- 内核入口命名约定：`origin_func_name`（例如 `x_attention_v2`）优先，然后回退到 `origin_func_name + "_kernel"`（例如 `x_attention_v2_kernel`）
- `x_attention_v2`（宿主函数，非 `@pl.jit`）作为 JIT 直调入口；`x_attention_v2_kernel`（`@pl.jit`）作为内置编译入口

### 已完成
- `x_attention_v2` 算子：场景 1（内核=`x_attention_v2`，宿主=`x_attention_v2_host`）已验证 — 构建成功，144 个测试在 8.15 秒内通过
- `x_attention_v2` 算子：场景 2（内核=`x_attention_v2_kernel`，宿主=`x_attention_v2`）已验证 — 构建成功，144 个测试在 7.89 秒内通过
- `ascendc_impl_build.py` 中的 `PYPTO_COMPILE_OP_API` 回退机制：先检查 `origin_func_name`，然后检查 `origin_func_name + "_kernel"`，如果都不存在则报错
- Golden 缓存重构：`golden_cache/x_attention/`（144 个文件）和 `golden_cache/x_flash_attention_infer/`（168 个文件）子目录；`test_x_attention_v2.py` 已更新为使用 `golden_cache/x_attention`
- `x_flash_attention_infer_v2` 算子：批次 1 内核（`paged_decode_float16_bs128_m16_nd_mf`）迁移自复现仓库，在 JIT 模式下验证（差异=0.0001）
- `x_flash_attention_infer_v2` 内置路径：编译通过（0 错误），但运行时崩溃（AICore 异常，核心 18/19 aivec 错误）— 根本原因疑似 `SetBlockDim` 值不正确
- `x_attention_v2` 性能：`maxPrompt = sharedTotal_ / batch_` 平铺修复产生了 -10.96% 的总内核时间提升（7927.8us → 7058.5us）
- `test/python_test/test_x_attention_v2_jit.py` 已创建并验证 — 144 个测试在 12.53 秒内通过（JIT 直调路径）

### 进行中
- `x_flash_attention_infer_v2` 内置运行时崩溃调试 — `SetBlockDim(28)` 可能需要为 AIV 子核心设置不同的值

### 受阻
- `x_flash_attention_infer_v2` aclnn 路径：AICore 异常（超时/陷阱错误，核心 18/19 aivec 错误）；JIT 模式运行正常，因此问题出在内置层（`SetBlockDim` 或张量布局传递）
- `x_flash_attention_infer_v2` 源文件当前不在工作树中（仅在 `/tmp/opencode/xfaiv2_build_block/` 有 `__pycache__`），需要从复现仓库重新迁移

### 关键决策
- 内核命名：`x_attention_v2_kernel`（`@pl.jit`）+ `x_attention_v2`（JIT 宿主）— 通过 `ascendc_impl_build.py` 中的回退机制实现
- `x_flash_attention_infer_v2`：1 个 `@pl.jit` 内核 + `tiling_key` + 数据类型覆盖所有 18 个复现变体（块大小/平铺 m 也可以使用 `tiling_key` `if` 分支，因为每个变体都是独立编译的）
- `x_flash_attention_infer_v2` 内核使用 `pl.Tensor` 参数（复现原始模式），而非 `pl.Ptr` + `make_tensor`（交付模式）
- `x_flash_attention_infer_v2` `def.cpp` 已简化：5 个输入（query, key_cache, value_cache, block_table, plan）+ 3 个属性（qHead, kvHead, scale）+ 1 个输出
- `x_flash_attention_infer_v2` `tiling.cpp` 已简化：仅 `SetBlockDim` + `SetTilingKey` + 工作空间（仅系统）
- `+ 16` 修复：`PIPE_V` 初始化必须在 `section_vector()` 内部，而非 `section_cube()`；跨核 Vec 缓冲区需要 `pad=pl.TilePad.min`

### 验证结果汇总

| 场景 | kernel 入口 | host 函数 | 编译 | 144 用例 | JIT 直调 |
|---|---|---|---|---|---|
| 1 (built-in) | `x_attention_v2_kernel` | `x_attention_v2` | ✅ SUCCESS | ✅ 144 passed 7.89s | ✅ 144 passed 12.53s |
| 2 (built-in) | `x_attention_v2` | `x_attention_v2_host` | ✅ SUCCESS | ✅ 144 passed 8.15s | — |

### 后续步骤
1. 恢复 `x_flash_attention_infer_v2` 工作时：修复 `SetBlockDim` 值以匹配 CANN 框架 AIV 子核心启动语义
2. 重新从复现仓库迁移 `x_flash_attention_infer_v2` 源文件到 `xllm_ops/x_flash_attention_infer_v2/`

### 关键上下文
- `x_attention_v2` `@pl.jit` 内核位于 `xllm_ops/x_attention_v2/op_kernel/x_attention_v2.py`（当前命名为 `x_attention_v2_kernel`）
- `x_attention_v2` JIT 宿主函数位于同文件（当前命名为 `x_attention_v2`）
- `ascendc_impl_build.py` `PYPTO_COMPILE_OP_API` 模板：`origin_func_name` = `optype_snake(op_type)`，新的回退机制会检查 `origin_func_name + "_kernel"`
- `x_flash_attention_infer_v2` 内核从复现仓库的 `paged_decode_float16_bs128_m16_nd_mf_kernel.py` 迁移，使用 `pl.Tensor` 参数
- JIT 启动语法：`k[None, NC, {'MergeFree': 1}, {'query': DataType.FP16, ...}](query, key_cache, ...)`
- `x_flash_attention_infer_v2` 在编译期间被移动到 `/tmp/opencode/xfaiv2_block`，以避免阻塞 `x_attention_v2` 构建
- `x_flash_attention_infer_v2.md` 记录了所有分析和进度（§8.10a 到 §8.10m）

### 相关文件
- `xllm_ops/x_attention_v2/op_kernel/x_attention_v2.py` — 内核入口 `x_attention_v2_kernel` + JIT 宿主 `x_attention_v2`
- `cmake/scripts/util/ascendc_impl_build.py` — 带有 `_kernel` 回退的 `PYPTO_COMPILE_OP_API` 模板
- `test/python_test/test_x_attention_v2.py` — 144 个测试用例，golden 缓存位于 `golden_cache/x_attention/`
- `test/python_test/test_x_attention_v2_jit.py` — JIT 直调路径测试，144 个用例，共享 golden 缓存
- `test/python_test/test_x_attention.py` — `test_x_attention_v2_jit.py` 的参考文件
- `xllm_ops/x_flash_attention_infer_v2/op_kernel/x_flash_attention_infer_v2.py` — 迁移自复现仓库的内核（批次 1）
- `xllm_ops/x_flash_attention_infer_v2/op_host/x_flash_attention_infer_v2_tiling.cpp` — 简化的平铺（仅 `SetBlockDim` + `TilingKey`）
- `xllm_ops/x_flash_attention_infer_v2/op_host/x_flash_attention_infer_v2_def.cpp` — 5 个输入 + 3 个属性 + 1 个输出
- `test/python_test/custom_ops.py` — `x_flash_attention_infer_v2_npu` 包装器
- `test/python_test/RegisterOps.cpp` — `x_flash_attention_infer_v2` 的 C++ 绑定
- `x_flash_attention_infer_v2.md` — `x_flash_attention_infer_v2` 的完整分析和进度文档
